...
public class BluetoothGpio8Demo {
    ...
    public static void main(String[] args) {
        ...
        if(args.length > 0){
            ...
            try {               
                ...
                /* Bluetooth GPIO8 Module Firmware version - device ID get & set                   */
                /************************************************************************** 
                    Send a command to output a logic high at GPIO 0. The command that is 
                    used to accomplish this action is "gpio set 0". It is important to send 
                ...
                */
                ...
                if(m.find())
                {
                 ...
                 /* GPIO8 group Commands                   */
                /*********************************************************************************************************** 
                    Send a command to output a logic iodir - iomask - writeall - readall at GPIO 0 to 7. The command that is 
                ...
                */                   
                ...
                /* GPIO8 IOdir - IOmask - Writeall - Readall */
                port.writeString("\r");               
                port.writeString("gpio iodir 00\r");
                System.out.println("Info: <gpio iodir XX> Command sent...");
                Thread.sleep(100);
                /* Flush the Serial port's RX & TX buffer. */
                port.purgePort(SerialPort.PURGE_RXCLEAR & SerialPort.PURGE_TXCLEAR);
                                
                port.writeString("\r");                
                port.writeString("gpio iomask ff\r");
                System.out.println("Info: <gpio iomask XX> Command sent...");
                Thread.sleep(100);
                /* Flush the Serial port's RX & TX buffer. */
                port.purgePort(SerialPort.PURGE_RXCLEAR & SerialPort.PURGE_TXCLEAR);
                
                 port.writeString("\r");                
                port.writeString("gpio writeall ff\r");
                System.out.println("Info: <gpio writeall XX> Command sent...");
                Thread.sleep(100);
                /* Flush the Serial port's RX & TX buffer. */
                port.purgePort(SerialPort.PURGE_RXCLEAR & SerialPort.PURGE_TXCLEAR);
