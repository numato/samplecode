//This C# demo code demonstrates how to use the 8 Channel Bluetooth Relay module and demonstrates its
...
namespace _8ChannelBluetoothRelayModuleDemo
{
    public partial class Form1 : Form
    {
        ...
        private void openportButton_Click(object sender, EventArgs e)
        {
            if ((serialPort1.IsOpen) == false)                                                                   //condition
            {
                ...
                try                                                                                              //do or act
                {
                    ...
                    relayWriteallButton.Enabled = true;                                                          //condition
                    relayReadallButton.Enabled = true;                                                           //condition
            ...
            else                                                                                                  //condition
            {
                try                                                                                               //do or act
                {
                    ...
                    relayWriteallButton.Enabled = false;                                                          //condition
                    relayReadallButton.Enabled = false;                                                           //condition
                    ...
                    relayWriteallBox.Text = String.Empty;                                                         //conditional case
                    relayReadallStstusBox.Text = String.Empty;                                                    //conditional case
                }
                catch                                                                                             //exception
                {
                    MessageBox.Show("Could Not Open Specified Port !");                                           //display
        ...
        private void adcButton_Click(object sender, EventArgs e)
        {
            ...
            {
                if (int.Parse(adcNumberBox.Text) < 2)                                                             //condition
                {
                    serialPort1.DiscardInBuffer();                                                                //discard input buffer
        ...
        private void gpiosetButton_Click(object sender, EventArgs e)
        {
                ...
                serialPort1.Open();
            {
                ...
                {
                    if (int.Parse(gpioNumberBox.Text) < 2)                                                    //condition
                    {
        ...
        private void gpioreadButton_Click(object sender, EventArgs e)
        {
                ...
                serialPort1.Open();
            {
                if (gpioNumberReadBox.Text.Length != 0)                                                       //condition
                {
                    ...
                    if (int.Parse(gpioNumberReadBox.Text) < 2)                                                //condition
                    {
                        try
                        {
                            serialPort1.DiscardInBuffer();                                                    //discard input buffer
        ...
        private void relayWriteallButton_Click(object sender, EventArgs e)
        {
            serialPort1.DiscardInBuffer();                                                                    	  //discard input buffer
            if (!serialPort1.IsOpen)                                                                              //condition
                serialPort1.Open();
            {
                if (relayWriteallBox.Text.Length == 2)                                                            //condition
                {
                    try                                                                                           //do or act
                    {
                        serialPort1.DiscardInBuffer();                                                            //discard input buffer
                        serialPort1.Write("relay writeall " + relayWriteallBox.Text + "\r");                      //writing "relay writeall XXXX" command to serial port
                        System.Threading.Thread.Sleep(200);                                                       //system sleep
                    }
                    catch                                                                                         //exception
                    {
        ...
        private void relayReadallButton_Click(object sender, EventArgs e)
        {
            serialPort1.DiscardInBuffer();                                                                    	  //discard input buffer
            if (!serialPort1.IsOpen)                                                                              //condition
                serialPort1.Open();
            {
                ...
                {
                    serialPort1.DiscardInBuffer();                                                                //discard input buffer
                    serialPort1.Write("relay readall\r");                                                         //writing "relay readall" command to serial port
                    System.Threading.Thread.Sleep(200);                                                           //system sleep
                    string response = serialPort1.ReadExisting();                                                 //read response string
